...
#define SET_SLEEP_TIMER(a)  a =  0x00ff; // alex added 0x186A0
...
deepSleepCore:
      ...
      //stick all our saved info onto stack since it's only temporary
      {
        ...
        SET_SLEEP_TIMER(wakeupSleepTmrCnt); // alex added
        if(!skipSleep) 
        {
          ...
          //&& alex un-commented following 2 lines:
          // wakeupSleepTmrCnt = SLEEPTMR_CNTH<<16;
          // wakeupSleepTmrCnt |= SLEEPTMR_CNTL;
          SET_SLEEP_TIMER(wakeupSleepTmrCnt);
