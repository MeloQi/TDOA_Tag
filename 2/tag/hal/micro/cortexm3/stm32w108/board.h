...
/**
 * @brief Define the number of LEDs in the PC998PR2
 */
#define LEDS_PC998PR2 1
...
/**
 * @brief Define the number of user buttons in the PC998PR2
 */
#define BUTTONS_PC998PR2 1
...
/**
 * @brief Flag to indicate if the board is equipped with 32kHz XTAL
 */
#define BOARD_HAS_LSE         (1 << 6)
/**
 * @brief Flag to indicate the GPIO status to switch on LEDs. If set the boards will switch on the LED when the GPIO is set to 1.
 */
#define BOARD_LED_NOT_INVERTED    (1 << 7)
...
/* leds definitions */
#define LED_Dn(n) (PORTx_PIN(boardDescription->io->leds[n].gpioPort, boardDescription->io->leds[n].gpioPin))
#define LED_D1 (boardDescription->leds>0 ? LED_Dn(0): DUMMY_LED)
#define LED_D3 (boardDescription->leds>1 ? LED_Dn(1): DUMMY_LED)
#define DUMMY_LED 0xff
#define LED_ON_VALUE ((boardDescription->flags & BOARD_LED_NOT_INVERTED) ? 1 : 0)
#define LED_OFF_VALUE ((boardDescription->flags & BOARD_LED_NOT_INVERTED) ? 0 : 1)
