...
// Clock switching for bootloader software activation
...
static void switchClock(void)
{
  ...
  int32u startTime = MAC_TIMER;
  // Wrapping is handled by the mask.
  while (((MAC_TIMER - startTime) & MAC_TIMER_MAC_TIMER_MASK) < (3000 / 2)) {}
  MAC_TIMER_CTRL = MAC_TIMER_CTRL_RESET;
...
void Reset_Handler(void)
{
  ...
  SCS_SHPR_15to12 = ((MED  <<SCS_SHPR_15to12_PRI_12_BIT) |   //Debug Monitor Handler
                     ...
                     (HIGH <<SCS_SHPR_15to12_PRI_15_BIT));   //halSysTickIsr
  ...
  if ((*((int32u *)RAM_BOTTOM) == IAP_BOOTLOADER_APP_SWITCH_SIGNATURE) && (*((int8u *)(RAM_BOTTOM+4)) == IAP_BOOTLOADER_MODE_UART)){
    int8u cut = *(volatile int8u *) (FIB_BOTTOM + 0x0798);
    int16u offset = 0;
  ...
  if ((*((int32u *)RAM_BOTTOM) == IAP_BOOTLOADER_APP_SWITCH_SIGNATURE)) {
    ...
    int32u bootAddress;
    *((int32u *)RAM_BOTTOM) = 0;
    if ((*((int8u *)(RAM_BOTTOM+4)) == IAP_BOOTLOADER_MODE_UART) || (*((int8u *)(RAM_BOTTOM+4)) == IAP_BOOTLOADER_MODE_OTA)) {
      bootAddress = (*((int8u *)(RAM_BOTTOM+4)) == IAP_BOOTLOADER_MODE_UART) ? FIB_BOTTOM : (FIB1_BOTTOM + 0x800);
      entryPoint = (EntryPoint)(*(int32u *)(bootAddress+4));
      setStackPointer(*((int32u *)bootAddress));
      entryPoint();
    }
  }
#endif
...
void halSysTickIsr() __attribute__ ((weak, alias("Default_Handler")));
