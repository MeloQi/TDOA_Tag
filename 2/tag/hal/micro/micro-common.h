...
#include "hal/micro/micro-common.h" // alex added
...
#ifdef DOXYGEN_SHOULD_SKIP_THIS
/** @brief Enumerations for the bootloader modes
 * - BOOTLOADER_MODE_UART
 *     Activate the internal UART bootloader
 * - BOOTLOADER_MODE_OTA
 *     Activate the over-the-air bootloader. It requires that the OTA bootloader
 *     is programmed in the device
 */
enum BootloaderModes
#else
typedef int8u BootloaderModes;
enum
#endif
{
 ...
 */
StStatus halBootloaderStart(BootloaderModes mode, int8u channel, int16u panId);
