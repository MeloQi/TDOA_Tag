...
{
    if (((regCmd[TRX_CHIRP_GEN_DATA].addrHi) == 0)
        && ((regCmd[TRX_CHIRP_GEN_DATA].addrLo) == 1)
        && ((regCmd[TRX_CHIRP_GEN_DATA].mask) == 1)
        && ((regCmd[TRX_CHIRP_GEN_DATA].operation) == 2)
        && ((regCmd[TRX_CHIRP_GEN_DATA].shadow) == 3)
        && ((regCmd[TRX_CHIRP_GEN_DATA].regId) == 5))
    {
        return TRUE;
    }
    return FALSE;
...
void NTRXSetRegister (NTRXCmdTE cmd, MyByte8T value)
{
     ...
     */
    if  (((regCmd[cmd].shadow) == NTRX_RW_REG)
      || ((regCmd[cmd].shadow) == NTRX_WO_REG))
    {

        reg = ntrxShadowReg[(regCmd[cmd].addrLo)];
    }
    else
    {
        reg = 0x00;
     ...
     */
    if ((regCmd[cmd].operation) == BOOL_OP) /* TRUE/FALSE value */
    {
        if (value == TRUE)
        {
            value = (regCmd[cmd].mask);
        }
    }
    else if (value != 0)
    {
         ...
         */
        value <<= (regCmd[cmd].operation);
    }

    /*
     * first reset the corresponding bits and then set only the new value
     */
    if ((regCmd[cmd].mask) != BOOL_OP)
    {
        reg &= ~((regCmd[cmd].mask));
        reg |= ((regCmd[cmd].mask) & value);
    }
    else
    {
        reg = value;
    ...
    if (((regCmd[cmd].shadow) == NTRX_RW_REG)
        || ((regCmd[cmd].shadow) == NTRX_WO_REG))
    {
        ntrxShadowReg[(regCmd[cmd].addrLo)] = reg;
    }

    NTRXSetRamIndex ((regCmd[cmd].addrHi));

    NTRXWriteSingleSPI ((regCmd[cmd].addrLo), reg);
}
...
void NTRXGetRegister (NTRXCmdTE cmd, MyByte8T *reg)
{
	...
	if ((regCmd[cmd].shadow) == NTRX_WO_REG)
	{
		*reg = ntrxShadowReg[(regCmd[cmd].addrLo)]; 	
	}
	else
	{
	    NTRXReadSPI ((regCmd[cmd].addrLo), reg, 1);
	}
...
void NTRXSetNRegister (NTRXCmdTE cmd, MyByte8T *data)
{
    ...
    NTRXWriteSPI ((regCmd[cmd].addrLo), data, (regCmd[cmd].mask));
}
...
void NTRXGetNRegister (NTRXCmdTE cmd, MyByte8T *reg)
{
    ...
    NTRXReadSPI ((regCmd[cmd].addrLo), reg, (regCmd[cmd].mask));
}
...
void NTRXProcessTable (const MyByte8T *table)
{
    ...
    while ( (table[index]) < CMD_ERROR)
    {
        switch ( (table[index]))
        {
            case TRX_DELAY:         timer =  (table[index + 1]);
                                    hwdelay (timer * 1000);
                                    break;
            ...
            case NA_AgcIntTime:
                                    for (i = 0; i < 2; i++)
                                    {
                                        data[i] =  (table[index + 1 + i]);
                                    }
                                    NTRXSetNRegister ( (table[index]), data);
                                    index++;
                                    break;
            ...
            default:                NTRXSetRegister ( (table[index]),
                                                     (table[index + 1]));
                                    break;
        }
        index += 2;
...
const MyByte8T NTRXInitTable2rx[]  =
{
...
const MyByte8T NTRXInitTable3rx[]  =
{
...
const MyByte8T NTRXInitTable4[]  =
{
    NA_DeviceSelect, NA_DeviceSelectBbRam0_C,
    TRX_DELAY, 2,
    CMD_ERROR
};
const unsigned char NAInitTxTable2tx[]  =
{
    NA_TxArq, TRUE,
...
const unsigned char NAInitTable3tx[]  =
{
    NA_EnableLO, FALSE,
...
//	if (firstTime++ != 0)
    ...
    //*传输速率初始化
    //NTRXSetRegister (NA_SymbolDur, 1);          //1000ns
    //NTRXSetRegister (NA_SymbolDur, 0);          //500ns  
    //NTRXSetRegister (NA_SymbolRate, 0x02);      //2 Mbps
...
void NTRXStartBasebandTimer (unsigned short startvalue)
{
